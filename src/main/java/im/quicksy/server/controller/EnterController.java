...
public class EnterController extends BaseController {
    ...
    private static final VerificationProvider VERIFICATION_PROVIDER =
            new TwilioVerificationProvider();

    public static TemplateViewRoute intro =
            (request, response) -> {
                HashMap<String, Object> model = new HashMap<>();
                model.put("fee", Payment.FEE);
                return new ModelAndView(model, "enter_index.ftl");
            };
    public static TemplateViewRoute getSendJabberVerification =
            (request, response) -> {
    ...
    public static Route postSendJabberVerification =
            (request, response) -> {
                ...
                if (input != null && !input.isEmpty()) {
                    try {
                        ...
                        String message =
                                String.format("Your Quicksy verification code is %s", code);
                        MyEjabberdApi.getInstance()
                                .sendChatMessage(
                                        from,
                                        jid.asBareJid().toEscapedString(),
                                        "Your Quicksy verification code",
                                        message);
                        ...
                        LOGGER.error(
                                "unable to send Jabber verification message - " + e.getMessage());
    ...
    public static TemplateViewRoute getVerifyJabber =
            (request, response) -> {
                Session session = request.session();
    ...
    public static Route postVerifyJabber =
            (request, response) -> {
                Session session = request.session();
                Jid jid = session.attribute("jid");
                String codeInSession = session.attribute("code");
    ...
    public static TemplateViewRoute getMakePayment =
            (request, response) -> {
    ...
    public static TemplateViewRoute getVoucher =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
    ...
    public static Route postVoucher =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
    ...
    public static TemplateViewRoute getSendSmsVerification =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
    ...
    private static VerificationProvider.Method verificationMethod(Request request) {
        try {
            return VerificationProvider.Method.valueOf(
                    request.queryParamOrDefault("method", "SMS"));
        } catch (IllegalArgumentException e) {
    ...
    public static Route postSendSmsVerification =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
                ...
                final VerificationProvider.Method method = verificationMethod(request);
                final String number =
                        input == null ? "" : CharMatcher.whitespace().removeFrom(input);
                if (E164_PATTERN.matcher(number).matches()) {
                    try {
                        Phonenumber.PhoneNumber phoneNumber =
                                PhoneNumberUtil.getInstance().parse(number, "us");
    ...
    public static TemplateViewRoute getVerifyNumber =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
    ...
    public static Route postVerifyNumber =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
                ...
                try {
                    if (code == null
                            || code.trim().isEmpty()
                            || !VERIFICATION_PROVIDER.verify(entry.getPhoneNumber(), code.trim())) {
                    ...
                    if (e.getCode() == TwilioVerificationProvider.PHONE_VERIFICATION_NOT_FOUND) {
                        ...
                        LOGGER.error(
                                "problem verifying sms authentication token"
                                        + e.getMessage()
                                        + " code="
                                        + e.getCode());
    ...
    public static TemplateViewRoute getFinished =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
            ...
            (request, response) -> {
                ...
                if (entry != null) {
                    ...
                    if (entry.isVerified() && (resetConfirmed == null || !resetConfirmed)) {
                        ...
                        response.redirect(
                                "/enter/send-number-verification/?method=" + method.toString());
                    }
                } else {
                    response.redirect("/enter/make-payment/");
                }
    ...
    public static TemplateViewRoute getConfirmReset =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
    ...
    public static TemplateViewRoute getConfirmDelete =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
    ...
    public static TemplateViewRoute getDelete =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
    ...
    public static Route getPayPalResult =
            (request, response) -> {
                ...
                if (PayPal.doExpressCheckoutPayment(token, payerId, payment.getTotal())) {
                    if (Database.getInstance()
                            .updatePaymentAndCreateEntry(payment, new Entry(payment.getOwner()))) {
    ...
    public static Route postCheckout =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
    ...
    public static TemplateViewRoute getPaymentReceived =
            (request, response) -> {
                final Session session = request.session();
                final Jid jid = getVerifiedJid(session);
                if (jid == null) {
