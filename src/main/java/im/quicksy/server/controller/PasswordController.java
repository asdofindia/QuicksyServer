...
public class PasswordController extends BaseController {
    ...
    private static final RateLimiter<InetAddress> PER_IP_RATE_LIMITER = RateLimiter.of(
            Strategy.of(Duration.ofMinutes(5), 16)
    );

    public static Route setPassword = (request, response) -> {
        ...
        final String pin;
        try {
            List<String> parameters = Splitter.on('\u0000').omitEmptyStrings().limit(2).splitToList(new String(BaseEncoding.base64().decode(authorization)));
            if (parameters.size() != 2) {
                throw new IllegalArgumentException("Invalid number of authorization parameters. Was " + parameters.size());
            }
        ...
        if (E164_PATTERN.matcher(username).matches() && PIN_PATTERN.matcher(pin).matches()) {
