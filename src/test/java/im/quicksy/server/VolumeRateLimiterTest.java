...
public class VolumeRateLimiterTest {
    ...
    public void hittingMultipleTimesWithSameWhat() throws VolumeLimiter.RetryInException {
        VolumeLimiter<Jid, String> volumeLimiter =
                new VolumeLimiter<>(Strategy.of(Duration.ofHours(1), 5));
    ...
    public void hittingOnceTooLarge() throws VolumeLimiter.RetryInException {
        ...
        volumeLimiter.attempt(
                Jid.of("person@domain.tld"), Arrays.asList("1", "2", "3", "4", "5", "6"));
    }
    ...
    public void expiry() throws VolumeLimiter.RetryInException, InterruptedException {
        VolumeLimiter<Jid, String> volumeLimiter =
                new VolumeLimiter<>(Strategy.of(Duration.ofSeconds(5), 5));
    ...
    private void rolling(long timeBetweenAttempts)
            throws InterruptedException, VolumeLimiter.RetryInException {
        List<Jid> persons =
                Arrays.asList(
                        Jid.of("a@example.com"), Jid.of("b@example.com"), Jid.of("c@example.com"));
        VolumeLimiter<Jid, String> volumeLimiter =
                new VolumeLimiter<>(Strategy.of(Duration.ofSeconds(5), 5));
        for (int i = 0; i <= 10; ++i) {
            for (Jid jid : persons) {
